/**
 * Code from Codeforces round 1024 div 1-2 Problem D
 * 
 * Contains code to find the number of inversions in an array.
 * 
 * Some observations:
 *  In an array with all distinct elements, one swap changes the number of
 *  inversions by an odd number; 
 *  If a[i] and a[j] are swapped, a[i] > a[j], and there are m elements between 
 *  a[i] and a[j] in the subarray a[i+1 : j-1], then after swap, the number of 
 *  inversions changes by (1 - 2*m).
 * 
 *  So, if we perform even number of swaps the parity of inversions remains same
 */
